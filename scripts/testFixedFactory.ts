...
async function main() {
  console.log("=== Testing FixedFactory ===");
  
  const TIMELOCK = "0xD54343A590e8fAAa1cb9ea9F1fddef4ABd365310";
  
  // Deploy FixedFactory
  const FixedFactory = await ethers.getContractFactory("FixedFactory");
  const factory = await FixedFactory.deploy();
  await factory.waitForDeployment();
  
  const factoryAddress = await factory.getAddress();
  console.log("FixedFactory deployed at:", factoryAddress);
  ...
  // Verify the FixedFactory
  console.log("\n=== Verifying FixedFactory ===");
  try {
    await hre.run("verify:verify", {
      address: factoryAddress,
      constructorArguments: [],
    });
    console.log("✅ FixedFactory verified on Etherscan");
